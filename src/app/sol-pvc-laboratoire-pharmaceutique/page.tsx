...
import { Baby, Building2, GraduationCap, Mail, Pill, Stethoscope, Store } from 'lucide-react'
...
const faq = [
  ...
  { q: "Quels documents pour le dossier qualité ?", a: "On remet en fin de chantier un dossier de traçabilité complet : fiches techniques, classement feu, classement A+ air intérieur, certificats de conformité européens, numéros de lot par zone, schéma de pose avec orientation des lés, photos avant et après. Le dossier accompagne directement vos audits internes ou clients, sans préparation supplémentaire. On fournit aussi les fiches de sécurité des colles, et un PV de fin de chantier signé par notre chef d'équipe et votre responsable site." },
  { q: "Peut-on poser pendant que le labo fonctionne ?", a: "Dans la majorité des cas, oui. On travaille zone par zone avec un balisage clair entre la partie en chantier et la partie en activité. Le chantier global est plus long, mais l'arrêt complet de la production est évité. On organise les transitions sur les pauses ou en fin de journée. On arrive en chaussures dédiées qu'on change au sas si la zone le demande. Sur les laboratoires les plus sensibles, il reste plus simple de viser un arrêt technique." },
  { q: "Combien de temps tient un sol PVC en labo ?", a: "Compter facilement quinze à vingt ans, avec un entretien quotidien normal et une bonne tenue visuelle sur toute la période. Dans un laboratoire d'analyse classique avec passage modéré, on dépasse régulièrement les vingt ans. Dans une zone de production cosmétique ou pharmaceutique avec palettes et chariots, on est plutôt sur quinze. La soudure à chaud entre les lés ne se dégrade pas dans le temps, c'est le revêtement lui-même qui finit par marquer." },
...
export default function SolPvcLaboratoirePharmaceutiquePage() {
  ...
  return (
    <div style={{ paddingTop: '72px' }}>
      <StickyDevisButton from="laboratoire-pharmaceutique" />
