...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `https://cbsols.fr/faq/${slug}` },
  }
}
...
export default async function FaqPage({ params }: Props) {
  ...
  const relatedSolutions = faq.relatedSolutions
    .map((s) => getSolutionBySlug(s))
    .filter(Boolean)

  const otherFaqs = faqs.filter((f) => faq.relatedFaqs.includes(f.slug))
      ...
      <div style={{ paddingTop: '72px' }}>
        ...
        <section style={{ padding: '5rem 2rem 4rem', borderBottom: '1px solid var(--border)', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ maxWidth: '900px', margin: '0 auto', position: 'relative' }}>
            ...
            <h1 style={{ fontFamily: 'var(--font-sans)', fontSize: 'clamp(1.75rem, 4vw, 3rem)', fontWeight: 800, color: 'var(--dark)', margin: '0 0 2rem', letterSpacing: '-0.03em', lineHeight: 1.1 }}>
              {faq.question}
            </h1>
        ...
        {/* Related FAQs */}
        {otherFaqs.length > 0 && (
          <section style={{ padding: '4rem 2rem', borderBottom: '1px solid var(--border)', backgroundColor: 'var(--bg-card)' }}>
            <div style={{ maxWidth: '900px', margin: '0 auto' }}>
              <div style={{ fontSize: '0.6rem', letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--terra)', marginBottom: '1.5rem' }}>Questions similaires</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0' }}>
                {otherFaqs.map((f) => (
                  <Link key={f.slug} href={`/faq/${f.slug}`} style={{ textDecoration: 'none', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1.125rem 0', borderBottom: '1px solid var(--border)' }}>
                    <div>
                      <div style={{ fontSize: '0.6rem', letterSpacing: '0.15em', textTransform: 'uppercase', color: 'var(--terra)', marginBottom: '0.3rem' }}>{f.category}</div>
                      <div style={{ fontSize: '0.9rem', fontWeight: 600, color: 'var(--dark)' }}>{f.question}</div>
                    </div>
                    <span style={{ fontSize: '0.65rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--terra)', flexShrink: 0, marginLeft: '1rem' }}>Lire →</span>
                  </Link>
                ))}
              </div>
            </div>
          ...
          <section style={{ padding: '5rem 2rem', borderBottom: '1px solid var(--border)' }}>
            <div style={{ maxWidth: '1280px', margin: '0 auto' }}>
              ...
              <h2 style={{ fontFamily: 'var(--font-sans)', fontSize: 'clamp(1.5rem, 2.5vw, 2rem)', fontWeight: 800, color: 'var(--dark)', margin: '0 0 2.5rem', letterSpacing: '-0.03em' }}>
                Nos solutions <em style={{ fontFamily: 'var(--font-serif)', fontStyle: 'italic', fontWeight: 300 }}>sur mesure</em>
              </h2>
                ...
                {relatedSolutions.map((solution) => solution && (
                  <Link key={solution.slug} href={`/solutions/${solution.slug}`} style={{ textDecoration: 'none' }}>
                    <div style={{ backgroundColor: 'var(--bg-card)', padding: '2rem', height: '100%' }} className="service-card">
                      ...
                      <p style={{ fontSize: '0.78rem', color: 'var(--muted)', margin: '0 0 1.5rem', lineHeight: 1.6 }}>{solution.tagline}</p>
