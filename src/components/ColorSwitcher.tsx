...
export function ColorSwitcher() {
  ...
  useEffect(() => {
    // Cleanup d'éventuels overrides hérités du slider beige précédemment introduit
    try { localStorage.removeItem('cbsols-beige-pct') } catch {}
    ...
    <div style={{ position: 'relative' }}>
      ...
      {open && (
        <>
          ...
          >
            <div style={{ fontSize: '0.55rem', letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: '0.25rem', paddingBottom: '0.5rem', borderBottom: '1px solid var(--border)', fontWeight: 600 }}>
              Accent
            </div>
            {palettes.map((p) => (
              <button
              ...
              >
                ...
                <span style={{ fontSize: '0.78rem', color: 'var(--dark-2)', fontWeight: active === p.id ? 600 : 400 }}>
                  {p.label}
                </span>
              </button>
            ))}
